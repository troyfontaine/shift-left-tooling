...
def get_commit_message(message_file: Optional[str] = None) -> str:
    """Get the message of the current commit or staged changes.

    Args:
        message_file: Optional path to commit message file (passed by git hooks).
                      If provided, reads from this file instead of git.
    ...
    """
    # If commit message file is provided (from git hook), read it directly
    if message_file:
        try:
            ...
            raise GitProviderError(
                f"Failed to read commit message file: {e}"
            ) from e
