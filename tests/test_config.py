...
from lib.config import AppConfig, BranchConfig, GitHubConfig, JiraConfig, get_config
...
class TestJiraConfig:
    ...
    def test_jira_config_with_url_and_token(self):
        ...
        with mock.patch.dict(
            os.environ,
            {"JIRA_URL": "https://jira.example.com", "JIRA_TOKEN": "test_token"},
        ):
            config = JiraConfig()
...
class TestBranchConfig:
    ...
    def test_branch_config_custom_pattern(self):
        ...
        with mock.patch.dict(os.environ, {"BRANCH_NAMING_PATTERN": r"^feature-.*"}):
            config = BranchConfig()
    ...
    def test_branch_config_invalid_pattern(self):
        ...
        with mock.patch.dict(os.environ, {"BRANCH_NAMING_PATTERN": "[invalid(regex"}):
            with pytest.raises(ValueError):
                BranchConfig()
