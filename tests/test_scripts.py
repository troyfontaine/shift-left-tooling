...
from scripts.enforce_protected_branches import (
    main as enforce_protected_branches_main,
)
from scripts.validate_jira_commit_link import (
    main as validate_jira_commit_link_main,
)
...
class TestValidateBranchName:
    ...
    def test_check_github_issue_exists_success(
        self, mock_extract, mock_origin, mock_github
    ):
    ...
    @mock.patch("scripts.validate_branch_name.get_git_origin")
    @mock.patch("scripts.validate_branch_name.extract_owner_repo")
    def test_check_github_issue_exists_error(
        self, mock_extract, mock_origin, mock_github
    ):
    ...
    @mock.patch(
        "scripts.validate_branch_name.sys.argv",
        ["validate_branch_name.py", "PROJ-123"],
    )
    ...
    @mock.patch(
        "scripts.validate_branch_name.sys.argv",
        ["validate_branch_name.py", "feature"],
    )
    @mock.patch("scripts.validate_branch_name.validate_jira_pattern")
...
class TestValidateBranchNaming:
    ...
    @mock.patch(
        "scripts.validate_branch_naming.sys.argv",
        ["validate_branch_naming.py", "PROJ-123"],
    )
    ...
    @mock.patch(
        "scripts.validate_branch_naming.sys.argv",
        ["validate_branch_naming.py", "feature"],
    )
    @mock.patch("scripts.validate_branch_naming.get_config")
    ...
    @mock.patch(
        "scripts.validate_branch_naming.sys.argv", ["validate_branch_naming.py"]
    )
    ...
    def test_extract_github_issue_gh_format(self):
        """Test extracting GitHub issue from gh format."""
        result = extract_github_issue("gh123-feature")
        assert result == "123"

    def test_extract_github_issue_gh_dash_format(self):
        """Test extracting GitHub issue from GH- format."""
        result = extract_github_issue("GH-456-feature")
        assert result == "456"

    def test_extract_github_issue_not_found(self):
        """Test when GitHub issue pattern not found."""
        result = extract_github_issue("feature-branch")
        assert result is None

    def test_extract_jira_ticket_success(self):
        """Test extracting Jira ticket from branch name."""
        result = extract_jira_ticket("PROJ-123-feature")
        assert result == "PROJ-123"

    def test_extract_jira_ticket_not_found(self):
        """Test when Jira ticket pattern not found."""
        result = extract_jira_ticket("gh123-feature")
        assert result is None

    @mock.patch("scripts.validate_branch_naming.JiraProvider")
    def test_validate_jira_ticket_exists(self, mock_jira_class):
        """Test validating Jira ticket exists."""
        mock_provider = mock.MagicMock()
        mock_provider.validate_ticket_exists.return_value = True
        mock_jira_class.return_value.__enter__.return_value = mock_provider

        result = validate_jira_ticket("PROJ-123")
        assert result is True

    @mock.patch("scripts.validate_branch_naming.JiraProvider")
    def test_validate_jira_ticket_not_exists(self, mock_jira_class):
        """Test validating Jira ticket that doesn't exist."""
        mock_provider = mock.MagicMock()
        mock_provider.validate_ticket_exists.return_value = False
        mock_jira_class.return_value.__enter__.return_value = mock_provider

        result = validate_jira_ticket("PROJ-999")
        assert result is False

    @mock.patch("scripts.validate_branch_naming.GitHubProvider")
    @mock.patch("scripts.validate_branch_naming.get_git_origin")
    @mock.patch("scripts.validate_branch_naming.extract_owner_repo")
    def test_validate_github_issue_exists(
        self, mock_extract, mock_origin, mock_github_class
    ):
        """Test validating GitHub issue exists."""
        mock_origin.return_value = "git@github.com:owner/repo.git"
        mock_extract.return_value = ("owner", "repo")
        mock_provider = mock.MagicMock()
        mock_provider.validate_issue_exists.return_value = True
        ...
        result = validate_github_issue("123")
        assert result is True

    @mock.patch(
        "scripts.validate_branch_naming.sys.argv",
        ["validate_branch_naming.py", "gh123"],
    )
    @mock.patch("scripts.validate_branch_naming.validate_github_issue")
    def test_main_valid_github_issue(self, mock_validate_github):
        """Test main with valid GitHub issue."""
        mock_validate_github.return_value = True
        result = validate_branch_naming_main()
        assert result == 0
    ...
    @mock.patch("scripts.validate_branch_naming.validate_jira_ticket")
    def test_main_valid_jira_ticket(self, mock_validate_jira):
        """Test main with valid Jira ticket."""
        mock_validate_jira.return_value = True
        result = validate_branch_naming_main()
        assert result == 0
...
class TestGetProtectedBranches:
    ...
    @mock.patch(
        "scripts.get_protected_branches.sys.argv", ["get_protected_branches.py"]
    )
    ...
    def test_main_github_success(
        self, mock_get_branches, mock_detect, mock_extract, mock_origin
    ):
    ...
    @mock.patch(
        "scripts.get_protected_branches.sys.argv",
        ["get_protected_branches.py", "--json"],
    )
    @mock.patch("scripts.get_protected_branches.get_git_origin")
    @mock.patch("scripts.get_protected_branches.extract_owner_repo")
    @mock.patch("scripts.get_protected_branches.detect_provider")
    ...
    def test_main_unknown_provider(
        self, mock_detect, mock_extract, mock_origin
    ):
    ...
    @mock.patch(
        "scripts.get_protected_branches.sys.argv",
        ["get_protected_branches.py", "--provider", "github"],
    )
    @mock.patch("scripts.get_protected_branches.get_git_origin")
    @mock.patch("scripts.get_protected_branches.extract_owner_repo")
    @mock.patch("scripts.get_protected_branches.get_protected_branches_github")
    def test_main_provider_override(
        self, mock_get_branches, mock_extract, mock_origin
    ):
...
class TestEnforceProtectedBranches:
    ...
    @mock.patch(
        "scripts.enforce_protected_branches.sys.argv",
    ...
    def test_main_branch_not_protected(
        self,
        mock_github_class,
        mock_detect,
        mock_extract,
    ...
    def test_main_unsupported_provider(
        self, mock_detect, mock_extract, mock_origin, mock_get_branch
    ):
...
class TestValidateJiraCommitLink:
    ...
    @mock.patch(
        "scripts.validate_jira_commit_link.sys.argv",
        ["validate_jira_commit_link.py"],
    )
    @mock.patch("scripts.validate_jira_commit_link.get_commit_message")
    @mock.patch(
        "scripts.validate_jira_commit_link.extract_jira_ticket_from_message"
    )
    ...
    @mock.patch(
        "scripts.validate_jira_commit_link.sys.argv",
        ["validate_jira_commit_link.py", "--check-exists"],
    )
    @mock.patch("scripts.validate_jira_commit_link.get_commit_message")
    @mock.patch(
        "scripts.validate_jira_commit_link.extract_jira_ticket_from_message"
    ...
    def test_main_check_exists_valid(
        self, mock_jira_class, mock_extract, mock_get_message
    ):
    ...
    @mock.patch("scripts.validate_jira_commit_link.JiraProvider")
    def test_main_check_exists_invalid(
        self, mock_jira_class, mock_extract, mock_get_message
    ):
    ...
    @mock.patch("scripts.validate_jira_commit_link.JiraProvider")
    def test_main_check_exists_error(
        self, mock_jira_class, mock_extract, mock_get_message
    ):
        """Test main with --check-exists when verification fails."""
        mock_get_message.return_value = "PROJ-123: Fix bug"
        mock_extract.return_value = "PROJ-123"
        mock_jira_class.return_value.__enter__.side_effect = Exception(
            "Jira error"
        )
...
class TestGenerateChangelog:
    ...
    def test_get_ticket_info_from_jira_success(self, mock_jira_class):
        ...
        mock_issue.permalink.return_value = (
    ...
    def test_generate_markdown_changelog_with_tickets(self):
        ...
        tickets = {"PROJ-123": {"summary": "Fix bug", "status": "DONE"}}
    ...
    def test_generate_plain_changelog_with_tickets(self):
        ...
        tickets = {"PROJ-123": {"summary": "Fix bug"}}
        result = generate_plain_changelog(tickets)
        assert "CHANGELOG" in result
        assert "PROJ-123" in result

    @mock.patch(
        "scripts.generate_changelog.sys.argv", ["generate_changelog.py"]
    )
    ...
    @mock.patch(
        "scripts.generate_changelog.sys.argv",
        ["generate_changelog.py", "--format", "json"],
    )
    @mock.patch("scripts.generate_changelog.get_commits_in_range")
    ...
    @mock.patch(
        "scripts.generate_changelog.sys.argv",
        ["generate_changelog.py", "--format", "plain"],
    )
    @mock.patch("scripts.generate_changelog.get_commits_in_range")
...
class TestValidateBranchNameMain:
    ...
    def test_main_github_check_exists_valid(
        self, mock_check, mock_github, mock_jira
    ):
    ...
    @mock.patch("scripts.validate_branch_name.validate_github_pattern")
    @mock.patch("scripts.validate_branch_name.check_github_issue_exists")
    def test_main_github_check_exists_invalid(
        self, mock_check, mock_github, mock_jira
    ):
